...
def _merge_datasets(img_path_13, gt_path_13, img_path_17, gt_path_17):
    dir_path_train_img = '../data/ICDAR2013/train_img/'
    dir_path_train_gt = '../data/ICDAR2013/train_gt/'
    map = pd.read_csv('../data/train_map.csv')
    num_13 = len(map['image'])
    for i in range(num_13):
      map['image'][i] = '../data/ICDAR2013/train_img/' + map['image'][i].split('/')[-1]
      map['label'][i] = '../data/ICDAR2013/train_gt/' + map['label'][i].split('/')[-1]
    total_img_files = map['image']
    total_gt_files = map['label']
